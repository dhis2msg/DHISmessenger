...
import org.dhis2.messaging.Utils.XMPP.IMUpdateUnreadMessages;
...
import org.dhis2.messaging.Utils.AsyncTasks.UnreadMessagesHandler;
...
public class HomeActivity extends FragmentActivity implements UnreadMessagesCallback, IMUpdateUnreadMessages {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        menuIcons = new int[]{R.drawable.ic_action_email, R.drawable.ic_action_chat, R.drawable.ic_action_picture,
                R.drawable.ic_action_person, R.drawable.ic_action_secure};
        ...
        new UnreadMessagesHandler(this).getAllUnreadMessages(this);
    }
    ...
    @Override
    public void updateUnreadMessages(final int amount) {
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < menuIcons.length; i++) {
                    HashMap<String, String> hm = new HashMap<String, String>();
                    if (i == 0)
                        hm.put("titles", menuTitles[i] + " | " + SharedPrefs.getUnreadMessages(getApplication()));
                    else if (i == 1)
                ...
                String[] key = {"icons", "titles"};
                int[] id = {R.id.listIcon, R.id.listTitle};
                drawerListView.setAdapter(new SimpleAdapter(getApplication(), list, R.layout.item_drawer, key, id));
    ...
    public void unreadMessagesLoadComplete() {
        //update drawer with amount of unread messages
        List<HashMap<String, String>> list = new ArrayList<HashMap<String, String>>();
        for (int i = 0; i < menuIcons.length; i++) {
            HashMap<String, String> hm = new HashMap<String, String>();
            if (i == 0)
